...
MARKITDOWN_WRAPPER_VERSION = "office-wrapper-v3"
ANYDOC_WRAPPER_VERSION = "office-anydoc-wrapper-v2"
# Backward-compatible alias used by existing callers and fixtures.
WRAPPER_VERSION = MARKITDOWN_WRAPPER_VERSION
MARKITDOWN_VERSION = "0.1.6"
ANYDOC_VERSION = "0.1.9"
OFFICE_FORMATS = ("docx", "pptx", "xlsx")
ANYDOC_FORMATS = tuple(
    sorted(
        {
            *OFFICE_FORMATS,
            "csv",
...
OFFICE_ENGINES = ("anydoc", "markitdown")
...
class AnyDocConverter:
    """Local-only anydoc wrapper over a request-scoped, preflighted file."""

    name = "anydoc"
    version = ANYDOC_VERSION
    wrapper_version = ANYDOC_WRAPPER_VERSION

    def __init__(self, engine: Any | None = None):
        if engine is None:
            import anydoc

            installed = distribution_version("firecrawl-anydoc")
            if installed != ANYDOC_VERSION:
                raise RuntimeError(
                    f"firecrawl-anydoc version {installed} does not match {ANYDOC_VERSION}"
                )
            engine = anydoc
        self._engine = engine

    def convert(self, source: Path, source_format: str) -> str:
        if source_format not in ANYDOC_FORMATS:
            raise OfficeError("unsupported_format", f"unsupported anydoc format: {source_format}")
        try:
            # The path is generated by this sidecar beneath its private request
            # directory. It never comes from multipart metadata or document
            # content, and anydoc performs no network access.
            text = self._engine.to_markdown(source)
        except Exception as exc:
            convert_error = getattr(self._engine, "ConvertError", ())
            if convert_error and isinstance(exc, convert_error):
                error_name = type(exc).__name__.removesuffix("Error").lower()
                status_code = 413 if error_name == "resourcelimit" else 422
                raise OfficeError(
                    f"anydoc_{error_name or 'conversion'}",
                    "anydoc could not produce meaningful Markdown",
                    status_code,
                ) from exc
            raise OfficeError(
                "anydoc_conversion_failed",
                "anydoc conversion failed",
                500,
            ) from exc
        if not isinstance(text, str):
            raise OfficeError("anydoc_invalid_result", "anydoc returned no Markdown")
        return _normalize_markdown(text)


def office_converter(engine: str) -> Converter:
    normalized = engine.strip().lower()
    if normalized == "markitdown":
        return MarkItDownConverter()
    if normalized == "anydoc":
        return AnyDocConverter()
    raise RuntimeError(f"unsupported RAG parser Office engine: {engine}")


def office_converter_descriptor(engine: str) -> ParserDescriptor:
    normalized = engine.strip().lower()
    if normalized == "markitdown":
        return ParserDescriptor(
            "markitdown", MARKITDOWN_VERSION, MARKITDOWN_WRAPPER_VERSION
        )
    if normalized == "anydoc":
        return ParserDescriptor("anydoc", ANYDOC_VERSION, ANYDOC_WRAPPER_VERSION)
    raise RuntimeError(f"unsupported RAG parser Office engine: {engine}")


def office_formats(engine: str) -> tuple[str, ...]:
    normalized = engine.strip().lower()
    if normalized == "markitdown":
        return OFFICE_FORMATS
    if normalized == "anydoc":
        return ANYDOC_FORMATS
    raise RuntimeError(f"unsupported RAG parser Office engine: {engine}")


def converter_descriptor(converter: Converter) -> ParserDescriptor:
    # Test converters written before the engine abstraction intentionally
    # retain the original MarkItDown contract unless they declare otherwise.
    name = str(getattr(converter, "name", "markitdown")).strip()
    version = str(getattr(converter, "version", MARKITDOWN_VERSION)).strip()
    wrapper = str(
        getattr(converter, "wrapper_version", MARKITDOWN_WRAPPER_VERSION)
    ).strip()
    if not name or not version or not wrapper:
        raise RuntimeError("Office converter descriptor is incomplete")
    return ParserDescriptor(name, version, wrapper)
...
def _instrument_office_source(
    ...
    with zipfile.ZipFile(source) as archive:
        ...
        slide_tokens: dict[str, str] = {}
        if source_format == "pptx" and converter_name == "anydoc":
            # anydoc follows the presentation relationship order but does not
            # expose a slide boundary in Markdown. Prefix the first visible
            # text run with a request-scoped marker so output can be split
            # back into canonical slide units. Empty slides safely fall back
            # to the existing conservative XML partition path.
            for index, (part, unit_id, _) in enumerate(units, 1):
                root = load_root(part)
                first_text = next(
                    (
                        node
                ...
                if first_text is None:
                    continue
                token = f"BKCRABSLIDE{sentinel_nonce}{index:04d}TOKEN"
                first_text.text = token + str(first_text.text)
                slide_tokens[unit_id] = token
...
def _rewrite_converter_images(
    ...
    # Office converters may emit sentinels for some embedded objects (notably
    # Visio OLE previews) as plain text instead of Markdown image alt text. Replace only
...
def _partition_markdown(
    ...
    if source_format == "pptx":
        ...
        for _, unit_id, _ in units:
            token = slide_tokens.get(unit_id, "")
            if not token:
                break
            match = re.search(re.escape(token), markdown)
            if match is None:
                break
            token_matches.append((match.start(), match.end(), unit_id))
        if len(token_matches) == len(units) and token_matches == sorted(token_matches):
            values: dict[str, str] = {}
            for index, (_, end, unit_id) in enumerate(token_matches):
                section_end = (
                    token_matches[index + 1][0]
                    if index + 1 < len(token_matches)
                    else len(markdown)
                )
                values[unit_id] = _normalize_markdown(markdown[end:section_end])
            return values, []
    ...
    return fallback, [
        ManifestWarning(
            ...
            message="Office 转换器输出无法稳定分段，已按 slide/sheet 保守提取可见文本",
            location=None,
            degraded=True,
        )
    ]
    ...
    for sentinel in sentinels:
        ...
        warnings.append(
            ManifestWarning(
                ...
                message="明确代码样式未能通过固定转换器 hook 定位，已保留普通文本",
                location=None,
                degraded=True,
            )
        )
...
def _prepare_anydoc_speaker_notes(markdown: str, speaker_notes: str) -> str:
    """Translate anydoc's native blockquote notes into the shared notes hook."""

    if not speaker_notes.strip():
        return markdown
    quoted = "\n".join(
        f"> {line}" if line else ">" for line in speaker_notes.strip().splitlines()
    )
    if markdown.count(quoted) != 1:
        return markdown
    return _normalize_markdown(markdown.replace(quoted, f"### Notes:\n\n{speaker_notes}", 1))
            ...
            if valid:
                ...
                warnings.append(
                    ManifestWarning(
                        ...
                        message="转换器表格未通过保守 GFM 校验，已保留单元格文字",
                        location=location,
                        degraded=True,
                    )
                )
...
def build_anydoc_bundle(
    *,
    ...
    """Build the shared bundle contract for non-OOXML anydoc formats."""
    parser_descriptor = converter_descriptor(converter)
    if parser_descriptor.name != "anydoc" or source_format not in ANYDOC_FORMATS:
        raise OfficeError(
            "unsupported_format", f"unsupported anydoc format: {source_format}"
        )
    location = SourceLocation("document", 0, "Document")
    markdown = _normalize_markdown(converter.convert(original_source, source_format))
    markdown, warnings = _validate_gfm_tables(markdown, location)
    entry_path = "units/0001.md"
    payload = PayloadEntry.from_bytes(
        entry_path,
    ...
    manifest = Manifest(
        ...
        parser=parser_descriptor,
        entries=(payload.descriptor(),),
        units=(MarkdownUnit("unit_document_0000", location, entry_path),),
        assets=(),
        attachments=(),
        occurrences=(),
        pages=(),
    ...
    *,
    ...
    with zipfile.ZipFile(sanitized_source) as archive:
        unit_markdown, partition_warnings = _partition_markdown(
            ...
            instrumented.slide_tokens,
        )
        if source_format == "pptx":
            notes_by_unit = {
                unit_id: _pptx_speaker_notes_text(archive, part, relationships)
    ...
    if source_format == "pptx":
        for _, unit_id, location in unit_sources:
            speaker_notes = notes_by_unit.get(unit_id, "")
            if parser_descriptor.name == "anydoc":
                unit_markdown[unit_id] = _prepare_anydoc_speaker_notes(
                    unit_markdown.get(unit_id, ""), speaker_notes
                )
            value, value_warnings = _normalize_speaker_notes(
                unit_markdown.get(unit_id, ""),
                has_speaker_notes=bool(speaker_notes),
                location=location,
            )
                ...
                for item, _ in missing:
                    ...
                    manifest_warnings.append(
                        ManifestWarning(
                            ...
                            message=(
                                f"图片锚定于{anchor}；{parser_descriptor.name} "
                                f"{parser_descriptor.version} 无稳定行级 hook，"
                                "已放入该工作表的相关图片小节"
                            ),
                            location=location,
                            degraded=True,
