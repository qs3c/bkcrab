...
export function ResultsPanel({ runs }: { runs: RAGEvalRun[] }) {
  ...
  const fallbackCount = cases.filter((item) => caseMetrics(item).some((metric) => metric.Details?.rerankerFallback === true)).length;
  ...
  return <div className="space-y-6">
    ...
    {run && <>
      ...
      <Card><CardHeader><CardTitle>Case drill-down</CardTitle><CardDescription>contexts 默认折叠；展开后可检查 raw recall/rerank score、过滤原因和逐项指标原因。</CardDescription></CardHeader><CardContent className="space-y-3">{cases.map((item) => { const metrics = caseMetrics(item); return <details key={item.caseId} className="rounded-md border p-3"><summary className="cursor-pointer"><span className="font-mono text-xs">{item.caseId}</span> <Badge className="ml-2" variant="outline">{item.status}</Badge> <span className="ml-2 text-xs text-muted-foreground">{item.latencyMs} ms</span></summary><div className="mt-3 space-y-3 text-sm">{item.errorCode && <div className="rounded-md bg-destructive/10 p-3 text-destructive"><p className="font-medium">{item.errorCode}</p>{item.errorMessage && <p className="mt-1 break-words text-xs">{item.errorMessage}</p>}</div>}<p className="whitespace-pre-wrap">{item.response || "无回答"}</p><div>{metrics.length ? metrics.map((metric) => <div key={`${metric.Name}-${metric.Version}`} className="flex justify-between border-t py-2"><span>{metric.Name} · {metric.Status}</span><span>{formatNumber(metric.Value, 4, metric.Reason || "—")}</span></div>) : <p className="text-xs text-muted-foreground">无评分结果</p>}</div><details><summary className="cursor-pointer text-muted-foreground">展开 contexts / raw scores / filter reasons</summary><pre className="mt-2 max-h-80 overflow-auto rounded bg-muted p-3 text-xs">{JSON.stringify(item.contexts, null, 2)}</pre></details></div></details>; })}</CardContent></Card>
    </>}
  </div>;
}
