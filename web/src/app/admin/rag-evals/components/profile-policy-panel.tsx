...
import { createRAGEvalProfile, getRAGCapabilities, getRAGPolicies, promoteRAGIngestionPolicy, promoteRAGRuntimePolicy, rollbackRAGRuntimePolicy, type RAGCapabilities, type RAGEvalProfile, type RAGEvalRun, type RAGParserEngine, type RAGPolicyAuditDTO, type RAGPolicyRecordDTO, type RAGSparseAnalyzer } from "@/lib/api";
...
const defaultProfile = JSON.stringify({
  ingestion: { version: 0, chunkSize: 512, chunkOverlap: 64, sparseAnalyzer: "chinese", parseMode: "standard", enrichmentEnabled: false, documentAI: {}, embedding: { contractFingerprint: "", model: "", dims: 1024 } },
...
function selectedSparseAnalyzer(value: string): RAGSparseAnalyzer | "" {
  try {
    const profile = JSON.parse(value) as { ingestion?: { sparseAnalyzer?: unknown } };
    const analyzer = profile.ingestion?.sparseAnalyzer;
    if (analyzer === undefined || analyzer === "") return "chinese";
    return analyzer === "chinese" || analyzer === "english" || analyzer === "multilingual" ? analyzer : "";
  } catch { return ""; }
}

function withSparseAnalyzer(value: string, sparseAnalyzer: RAGSparseAnalyzer): string {
  const profile = JSON.parse(value) as { ingestion?: Record<string, unknown> };
  profile.ingestion = { ...(profile.ingestion ?? {}), sparseAnalyzer };
  return JSON.stringify(profile, null, 2);
}
...
export function ProfilePolicyPanel({ profiles, runs, onProfileChanged }: { profiles: RAGEvalProfile[]; runs: RAGEvalRun[]; onProfileChanged: () => Promise<void> }) {
  ...
  function chooseSparseAnalyzer(sparseAnalyzer: RAGSparseAnalyzer) {
    try { setProfileJSON((current) => withSparseAnalyzer(current, sparseAnalyzer)); setError(""); }
    catch { setError("Profile JSON 无法解析，请先修复 JSON 后再选择 Sparse 分词器"); }
  }
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    <Card><CardHeader><CardTitle>实验 Profile</CardTitle><CardDescription>从已有 Profile 创建副本，分别固定 AnyDoc/MarkItDown；版本保存后不可变，运行结果会记录建库耗时。</CardDescription></CardHeader><CardContent><form className="space-y-3" onSubmit={saveProfile}>
      ...
      <Label>文本语言 / Sparse 分词</Label><div className="grid gap-2 sm:grid-cols-3">{sparseAnalyzerOptions.map((option) => <Button key={option.value} type="button" className="h-auto min-h-16 flex-col items-start whitespace-normal text-left" variant={selectedSparseAnalyzer(profileJSON) === option.value ? "default" : "outline"} onClick={() => chooseSparseAnalyzer(option.value)}><span>{option.label}</span><span className="text-xs opacity-75">{option.description}</span></Button>)}</div>
      <p className="text-xs text-muted-foreground">影响 Milvus BM25 稀疏检索。发布 IngestionPolicy 后，新知识库直接采用；存量知识库会在主动策略同步时建立新 generation，完成前继续使用旧索引。</p>
