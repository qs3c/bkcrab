...
export function DatasetsPanel({ capabilities, datasets, onChanged }: {
  ...
  return <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
    <div className="space-y-6">
      <CatalogImportPanel datasets={datasets} maxRunCases={capabilities?.maxRunCases || 0} onChanged={onChanged} />
      <Card>
        <CardHeader><CardTitle>自定义数据集（高级）</CardTitle><CardDescription>仅在内置目录不适用时，手动创建并上传 Canonical Dataset。</CardDescription></CardHeader>
        <CardContent><form className="space-y-4" onSubmit={createDataset}>
          ...
          <Button type="submit" className="w-full" disabled={busy || !capabilities?.enabled || !name.trim()}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}创建</Button>
        </form></CardContent>
      </Card>
      <Card>
        <CardHeader><CardTitle>手动导入 Canonical 版本</CardTitle><CardDescription>兼容旧流程；浏览器会编码文件，较大语料请使用上方内置目录。</CardDescription></CardHeader>
        <CardContent><form className="space-y-4" onSubmit={importVersion}>
          ...
          <Button type="submit" className="w-full" disabled={busy || !selected || !manifest}><Upload className="mr-2 h-4 w-4" />校验并导入</Button>
        </form></CardContent>
      </Card>
    </div>
    <div className="space-y-6">
