...
func (s *staticEvalJudge) Chat(ctx context.Context, _ []provider.Message, tools []provider.Tool, _ string, _ int, _ float64) (*provider.Response, error) {
	s.tools = tools
	s.sessionID = provider.SessionIDFromContext(ctx)
...
func TestRAGEvalJudgeProxyRequiresInternalTokenAndForwardsOwner(t *testing.T) {
	...
	firstSession := judge.sessionID
	for _, owner := range []string{"admin", "another-admin"} {
		retry := httptest.NewRequest(http.MethodPost, request.URL.String(), strings.NewReader(body))
		retry.Header.Set("Authorization", "Bearer internal-secret")
		retry.Header.Set("X-BkCrab-Eval-Owner", owner)
		rr := httptest.NewRecorder()
		server.handleRAGEvalJudgeProxy(rr, retry)
		if rr.Code != http.StatusOK || (judge.sessionID == firstSession) != (owner == "admin") {
			t.Fatal("judge retries must be stable and isolated by owner")
		}
	}
}
