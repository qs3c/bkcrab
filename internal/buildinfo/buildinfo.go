...
// (rather than cmd/bkclaw/main) so internal/* code — notably the agent
...
// Version is the BkClaw release tag (e.g. "v0.4.2") set by the
...
// IsHostedDeploy reports whether this bkclaw process is running in a
// hosted/multi-tenant deployment (cloud) versus a self-hosted single-
// operator install. Driven by the BKCLAW_DEPLOY env var:
//
...
// the most common case (developer running bkclaw on their own
...
// osDeployVar reads BKCLAW_DEPLOY and normalizes it. Lowercased so
...
// flow, `bkclaw upgrade`, `~/Downloads` triage) sets the env var
