...
// 通过链接时的 -ldflags -X 填充。独立成包（而非放在 cmd/bkcrab/main）
...
// Version 是 BkCrab 的发布标签（例如 "v0.4.2"），由 Makefile 通过
...
// IsHostedDeploy 报告当前 bkcrab 进程是否运行在托管/多租户部署（云）
// 环境，而非自托管单操作员安装。由 BKCRAB_DEPLOY 环境变量控制：
//
//	BKCRAB_DEPLOY=hosted        → IsHostedDeploy() == true
//	BKCRAB_DEPLOY=self-hosted   → false
//	（未设置或其它值）           → false（默认 = 自托管）
//
// 操作员在其云部署清单中设置此值（k8s values.yaml、docker-compose env 等）。
// 默认自托管匹配最常见的情况（开发者在自己的笔记本上运行 bkcrab），
...
// osDeployVar 读取 BKCRAB_DEPLOY 并归一化。小写化处理，使大小写
// 变体不会静默绕过 hosted 标志。
func osDeployVar() string {
	return strings.ToLower(strings.TrimSpace(os.Getenv("BKCRAB_DEPLOY")))
}

// IsHostExecAllowed 报告 agent 运行时是否应注册 `host_exec` 逃生舱工具。
// 仅当操作员通过 BKCRAB_ALLOW_HOST_EXEC=1（或 "true"/"yes"）显式选择加入，
// 且不是托管多租户部署时才返回 true。
//
...
// （单用户笔记本开发流程、`bkcrab upgrade`、`~/Downloads` 整理）在部署时
// 设置环境变量；其他人则获得更安全的沙箱或无沙箱行为。
func IsHostExecAllowed() bool {
	if IsHostedDeploy() {
		return false
	}
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BKCRAB_ALLOW_HOST_EXEC")))
