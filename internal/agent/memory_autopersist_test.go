...
func TestAutoPersistMemoryPromptUsesOnlyUserAndAssistantText(t *testing.T) {
	...
	if err := AutoPersistMemory(context.Background(), mgr, prov, "fake-model", groups); err != nil {
		t.Fatalf("AutoPersistMemory: %v", err)
	}
	for _, want := range []string{"USER_SHOULD_APPEAR", "ASSISTANT_SHOULD_APPEAR"} {
		if !strings.Contains(prov.gotPrompt, want) {
			t.Fatalf("prompt missing %q:\n%s", want, prov.gotPrompt)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(prov.gotPrompt, forbidden) {
			t.Fatalf("prompt included %q:\n%s", forbidden, prov.gotPrompt)
		}
	}
}
