...
func TestAppendTurnAnchorReturnsSeqAndStatus(t *testing.T) {
	...
	var firstStatus, latestStatus string
	if err := db.db.QueryRowContext(ctx,
		`SELECT turn_status FROM session_messages WHERE user_id=? AND agent_id=? AND session_key=? AND seq=?`,
		uid, agent, sk, seq0).Scan(&firstStatus); err != nil {
		t.Fatalf("read status: %v", err)
	}
	if err := db.db.QueryRowContext(ctx,
		`SELECT turn_status FROM session_messages WHERE user_id=? AND agent_id=? AND session_key=? AND seq=?`,
		uid, agent, sk, seq1).Scan(&latestStatus); err != nil {
		t.Fatalf("read latest status: %v", err)
	}
	if firstStatus != "" || latestStatus != "running" {
		t.Fatalf("turn statuses first=%q latest=%q, want superseded/running", firstStatus, latestStatus)
	}
}
