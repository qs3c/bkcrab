...
func mysqlMigrationSQL() []string {
	return []string{
		...
		`CREATE TABLE IF NOT EXISTS image_generation_batches (
			...
			KEY idx_image_generation_batches_status (status, updated_at, id),
			KEY idx_image_generation_batches_workspace (agent_id, workspace_session_id, created_at)
		) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
		`CREATE TABLE IF NOT EXISTS image_generation_tasks (
			...
			CONSTRAINT chk_image_generation_tasks_requested_count CHECK (requested_count BETWEEN 1 AND 4),
			...
			KEY idx_image_generation_tasks_user_sequence (user_id, sequence_id),
			KEY idx_image_generation_tasks_expired (status, lease_until, sequence_id),
			KEY idx_image_generation_tasks_batch (batch_id, status, item_index, chunk_index)
		) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
