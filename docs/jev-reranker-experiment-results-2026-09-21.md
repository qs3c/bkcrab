...
新绑定是 `deepseek-official/deepseek-v4-flash`，当前评测用户默认模型已指向该绑定。官方 API 的实测返回 model 为 `deepseek-flash`；[官方说明](https://api-docs.deepseek.com/quick_start/pricing/)表明旧模型名现由 V4.1 Flash 承接。15 份 smoke 答案已重新生成成功。现有 judge 代理不保留强制 tool_choice，故改用独立 Docker 评审直连官方 API、关闭 judge thinking，复用相同 Ragas 指标与 prompt；还在验证评审传输稳定性。旧失败记录保留，不能与恢复后的指标混算。
...
2. 完成官方渠道 5 题三组的评审 smoke，再生成正式三组答案与 Ragas 评审。
