...
func pluginInstallCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Install a plugin from BkCrab Hub, GitHub, npm, or local path",
		Long: `Install a plugin. The source is auto-detected:

  bkcrab plugins install telegram                        # BkCrab Hub
  bkcrab plugins install github.com/user/repo            # GitHub repo
  bkcrab plugins install @ollama/web-search              # npm plugin (bridged)
  bkcrab plugins install ./my-plugin                     # local directory`,
...
func installFromGitHub(source, pluginsDir string) error {
	...
	// 克隆到临时目录
	tmpDir, err := os.MkdirTemp("", "bkcrab-plugin-*")
	if err != nil {
		return err
	}
...
func installFromHub(name, pluginsDir string) error {
	...
	fmt.Printf("Installing %q from BkCrab Hub...\n", name)
	...
	// 查找顶级目录（名称可能为 bkcrab-main、bkcrab-v0.16.0 等）
	...
	if _, err := os.Stat(pluginDir); os.IsNotExist(err) {
		return fmt.Errorf("plugin %q not found in BkCrab Hub", name)
	}
...
func installFromNpm(pkg, pluginsDir string) error {
	...
	pluginID = strings.TrimPrefix(pluginID, "bkcrab-")

	// 1. 在临时目录中执行 npm install 以检查包
	tmpDir, err := os.MkdirTemp("", "bkcrab-npm-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmpDir)
	...
	if _, err := os.Stat(proxyJS); os.IsNotExist(err) {
		fmt.Println("Installing plugin-bridge from BkCrab Hub...")
...
`
	...
	if testErr != nil && toolCount == 0 {
		if hasChannel {
			return fmt.Errorf("cannot install %s: this is a channel plugin that requires a separate runtime. Consider writing a native BkCrab plugin instead", pkg)
		}
		return fmt.Errorf("cannot install %s: plugin is not compatible with BkCrab bridge", pkg)
	}
