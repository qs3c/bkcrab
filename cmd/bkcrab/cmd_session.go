...
from the LOCAL bkcrab.db — "online" sessions need their DB pulled
down first (rsync ~/.bkcrab/bkcrab.db from the host).

Examples:
  bkcrab session export http://localhost:18953/agents/agt_xxx/chat/s-yyy/
  bkcrab session export https://app.bkcrab.ai/agents/agt_xxx/chat/s-yyy -o /tmp/run.json`,
	...
	cmd.Flags().StringVarP(&outputPath, "output", "o", "",
		"output JSON path (default ~/.bkcrab/logs/<sessionKey>.json)")
	cmd.Flags().StringVar(&dbPath, "db", "",
		"legacy SQLite DB path (default: BKCRAB_STORAGE_* MySQL database)")
	return cmd
}
...
// ~/.bkcrab/logs/<sessionKey>.json。每个会话使用独立文件名，
...
func resolveOutputPath(explicit, sessionKey string) (string, error) {
	...
	return filepath.Join(home, ".bkcrab", "logs", sessionKey+".json"), nil
}
